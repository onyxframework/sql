class Atom
  module Model
    ...
    private macro define_initializer
      ...
      def initialize(*,
        {% for type in (MODEL_ATTRIBUTES + MODEL_REFERENCES).select { |t| !t["db_nilable"] && !t["db_default"] && !t["default_instance_value"] }.reject(&.["foreign"]) %}
          {{type["name"]}} : {{type["type"]}},
        {% end %}

        {% for type in (MODEL_ATTRIBUTES + MODEL_REFERENCES).select { |t| !t["db_nilable"] && !t["db_default"] && !t["default_instance_value"] }.select(&.["foreign"]) %}
          {{type["name"]}} : {{type["type"]}} | Nil = nil,
        {% end %}

        {% for type in (MODEL_ATTRIBUTES + MODEL_REFERENCES).reject { |t| !t["db_nilable"] && !t["db_default"] && !t["default_instance_value"] } %}
          {{type["name"]}} : {{type["type"]}}{{" | DB::Default.class".id if type["db_default"]}} = {{type["default_instance_value"]}},
        {% end %}
      )
        @explicitly_initialized = true

        {% for type in (MODEL_ATTRIBUTES + MODEL_REFERENCES) %}
          @{{type["name"]}} = {{type["name"]}}
        {% end %}
      end
    end
