  ...
  struct Query(T)
    ...
    def where(or : Bool = false, not : Bool = false, **values : **Values) : self forall Values
      {% for key, value in Values %}
        ...
        {% for type in (T::MODEL_ATTRIBUTES + T::MODEL_REFERENCES.select { |t| t["direct"] && !t["enumerable"] }) %}
        ...
        {% raise "Class '#{T}' doesn't have an attribute with key '#{key}' defined in its Schema eligible for 'Action::Model::Query(#{T})#where' call" unless found %}
      {% end %}
        ...
        values.each_with_index do |key, value, index|
          if value.nil?
            case key
              ...
              {% for type in T::MODEL_REFERENCES.select { |t| t["direct"] && !t["enumerable"] } %}
