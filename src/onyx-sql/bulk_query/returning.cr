module Onyx::SQL
  class BulkQuery(T)
    # Add `RETURNING` clause by either model field or reference or
    # explicit Char or String.
    #
          ...
          if value.is_a?(T::Field)
            ...
            {% for ivar in T.instance_vars.reject(&.annotation(Reference)) %}
              when .{{ivar.name}}?
                ...
                ensure_returning << "{{table.id}}.#{column}"
            {% end %}
            else
    ...
    def returning(klass : T.class)
      ensure_returning << {{T.annotation(Model::Options)[:table].id.stringify}} + ".*"
