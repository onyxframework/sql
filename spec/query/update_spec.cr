...
describe "Atom::Query#update" do
  it do
    uuid = UUID.random
    ref_uuid = UUID.random

    q = Atom::Query(User).new.update.set(name: "John", active: DB::Default).set(referrer: User.new(uuid: ref_uuid, name: "Jake"), updated_at: nil).where(uuid: uuid)
