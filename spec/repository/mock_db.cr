class MockDB
  ...
  getter latest_exec_params : Array(DB::Any | Array(DB::Any))? = nil

  def exec(sql, *params : DB::Any | Array(DB::Any))
    @latest_exec_sql = sql
    @latest_exec_params = params.to_a
    return DB::ExecResult.new(0, 0)
  end

  def exec(sql, params : Enumerable(DB::Any | Array(DB::Any))? = nil)
    @latest_exec_sql = sql
  ...
  getter latest_scalar_params : Array(DB::Any | Array(DB::Any))? = nil
  ...
  getter latest_query_params : Array(DB::Any | Array(DB::Any))? = nil

  def query(sql, *params : DB::Any | Array(DB::Any))
    @latest_query_sql = sql
    @latest_query_params = params.to_a
    ResultSet.new
  end

  def query(sql, params : Enumerable(DB::Any | Array(DB::Any))? = nil)
    @latest_query_sql = sql
...
module Core
  class Repository
    def initialize(@db : MockDB, @logger = Logger::Dummy.new)
    end
  end
end
